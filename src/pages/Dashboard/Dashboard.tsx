...
const Dashboard: React.FC = () => {
  ...
  const processAlerts = (rawAlerts: ApiAlert[]): Alert[] => {
    return rawAlerts.slice(0, 10).map((alert, index) => { // Show top 10
      ...
      return {
        ...
        timestamp: new Date(alert.Timestamp).toLocaleTimeString(), // Format time
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      const [deviceResult, alertResult] = await Promise.allSettled([
        api.get<DevicesApiResponse>('/device'),
        api.get<ApiAlert[]>('/device/alarm/notifications')
      ]);
  ...
  const deviceStatusData = useMemo(() => {
    ...
    devices.forEach(device => {
      ...
      // Add 'warning' / 'in progress' if your API supports it
  ...
  const osDistributionData = useMemo(() => {
    // This will find the top 3 OS types and group the rest
    const TOP_N = 3;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Recent Activity section is removed */}
    </div>
  );
};
