...
import GrafanaView from "./components/Monitoring/GrafanaView";
import ConfigurationManager from "./components/Settings/ConfigurationManager";
import GrafanaConfig from "./components/Settings/GrafanaConfig";
...
function App() {
  ...
  const handleConfigureL2Services = (device: Device) => {
    setCurrentView("nav-config-management");
    setSelectedDevice(device);
  };
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case "nav-inventory-devices":
        return <DeviceList onDeviceSelect={handleDeviceSelect} onConfigureL2Services={handleConfigureL2Services} />;
      case "nav-topology":
        return (
          <TopologyScansView
      ...
      case "nav-monitoring-grafana":
        return <GrafanaView onClose={() => setCurrentView("nav-dashboard")} />;
      ...
      case "nav-settings-grafana":
        return (
          <GrafanaConfig onClose={() => setCurrentView("nav-dashboard")} />
        );
  ...
  const getPageSubtitle = () => {
    switch (currentView) {
      ...
      case "nav-monitoring-grafana":
        return "View Grafana dashboards";
      case "nav-config-management":
      ...
      case "nav-settings-grafana":
        return "Configure Grafana integration and dashboard settings";
      case "nav-settings-roles":
